...
class TestCallCommand:
    def test_call__invalid_target_raises_bad_parameter(self):
        """Call command raises BadParameter for an invalid SIP URI target."""
        from click.testing import CliRunner

        runner = CliRunner()
        result = runner.invoke(
            voip,
            ["sip", "sips:alice:secret@example.com", "call", "not-a-sip-uri"],
        )
        assert result.exit_code != 0
        assert "TARGET" in result.output

    def test_call__initiates_outbound_invite(self):
        """Call command registers, then sends an INVITE to the target."""
        import dataclasses
        ...
        _run_call_command(fake_connect_rtp, fake_connect_sip_once)

        assert any(b"INVITE" in data for data in sent_data)

    def test_call__bye_received_closes_session(self):
        """OutboundInviteTransaction.bye_received closes the SIP session."""
        import dataclasses
...
# ---------------------------------------------------------------------------
# Helpers used by TestCallCommand
# ---------------------------------------------------------------------------
...
def _run_call_command(
    ...
    """Invoke the ``call`` CLI command with patched transport helpers."""
    import voip.__main__ as main_module
    from click.testing import CliRunner
    ...
    try:
        result = CliRunner().invoke(
            voip,
            ["sip", "sips:alice:secret@example.com", "call", "sip:bob@biloxi.com"],
