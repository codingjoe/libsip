...
class TestRequest:
    ...
    def test_via_branch__with_branch(self):
        ...
        assert request.via_branch == "z9hG4bKabc123"

    def test_via_branch__fallback_to_call_id(self):
        """via_branch falls back to Call-ID when no branch is present (RFC 2543)."""
        request = Request(
            method="INVITE",
            uri="sip:bob@biloxi.com",
            headers={
                "Via": "SIP/2.0/UDP pc33.atlanta.com",
                "Call-ID": "legacy-call-1",
            },
        )
        assert request.via_branch == "legacy-call-1"
